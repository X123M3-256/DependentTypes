...
processDefinition (Axiom name ty) ctx=case Map.lookup name ctx of
					Just _ -> Left (DuplicateBinding name,(SourcePos 0 0,SourcePos 0 0))--TODO use actual source positions
					Nothing -> Right (Map.insert name (ty,Nothing) ctx)
processDefinition (Lemma name ty prf) ctx=case Map.lookup name ctx of
						Just _ -> Left (DuplicateBinding name,(SourcePos 0 0,SourcePos 0 0))
...
main::IO()
main=do
		parseResult<-(parseFile "naturalnumbers.txt")
		putStrLn (case parseResult of
							Left err -> parseErrorPretty err
							Right res -> case typecheck res (Map.fromList []) of
