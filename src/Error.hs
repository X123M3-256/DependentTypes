...
data ErrorType a=ParseError String|UndeclaredIdentifier String|TypeMismatch (Expr a) (Expr a) (Expr a)|ExpectedPiType (Expr a)|ExpectedSigmaType (Expr a)|ExpectedSumType (Expr a)|ExpectedEqType (Expr a)|ExpectedUniverse (Expr a) (Expr a)|DuplicateBinding String|FileNotFound String|ReplaceFailed String|TypeInferenceFailed (Expr a)
...
printError fname (ty,ctx,sr)=
		...
		let msg=
			case ty of
				...
				ReplaceFailed str		-> "Replace failed "++str
				TypeInferenceFailed expr	-> "Cannot infer type of \""++(printExpr ctx (simplify ctx expr))++"\" - type annotation required"
				...
				ExpectedEqType expr 		-> "Expected equality type but recieved:\n\n\t\t"++(printExpr ctx (simplify ctx expr))++"\n\n\t"
